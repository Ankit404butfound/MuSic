...
public class MusicHandler {
    ...
    public void playSong(String path, String songName, String artistAlbum) {
        try {
            player.stop();
            player.release();
            player = null;
                ...
                mediaPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                    ...
                    public void onCompletion(MediaPlayer mediaPlayer) {
                        if (Globals.loop_type == "loop_song") {
                            ...
                            playSong(path, songName, artistAlbum);
                        } else {
                            playNextSong();
...
//        NotificationManager.updateNotification(songData.song, songData.artistAlbum);
        ...
        playSong(songData.filename, songData.song, songData.artistAlbum);
    }

    public void updateColorsOfLastTv(){
        if (Globals.playedSongDataArray.size() > 0) {
            TextView txt_v = Globals.playedSongDataArray.get(Globals.playedSongDataArray.size() - 1).textView;
    ...
    public void playNextSong() {
        ...
        if (Globals.loop_type.matches("shuffle")) {
            ...
            if (length < 1) {
                ...
                Log.d("num", String.valueOf(randIndex));

                SongData nextSongData = Globals.songDataArray.get(randIndex);
                ...
                playSong(nextSongData.filename, nextSongData.song, nextSongData.artistAlbum);
